##### This code is developed to extract deep features using cnn, train and test lstm (label last frame)  ####
...
#label=np.load('BVZ0036_269pots_ordered_numerically_2_1667315.npz')
#    temp = TOT_270[i,2,:,:]
#    temp2 = cv2.resize(temp,(227,227))
#    TOT_270_r[i,0,:,:] = temp2-m[2]
#    TOT_270_r[i,:,:,:] = np.expand_dims(TOT_270_r[i,:,:,:], axis=0)
...
#            TD_model.add(Dropout(0.5))
    TD_model.add(LSTM(256,return_sequences=True ))
    TD_model.add(Dropout(0.5))
    TD_model.add(LSTM(256,return_sequences=False ))
    TD_model.add(Dropout(0.5))
...
#print(ind_test_0)
#print(ind_val_0)
#print(Tray[ind_test_0])
#print(BVZ[ind_test_0])
#print(y1[ind_test_0]-1)
#print(Tray[ind_val_0])
#print(BVZ[ind_val_0])
#print(y1[ind_val_0]-1)
    ...
    for nf in range(0, newYval.shape[0]):
        for df in range(4) :
            if newYval[nf,df]==1:
                ny[nf]=df
    print(ny)
    checkpointer = ModelCheckpoint(filepath="./tmp/weights_SNP_7_1.hdf5", verbose=1, save_best_only=True)
...
#            LSTM_model.save_weights("./tmp/final_weights_SNP1.hdf5",overwrite=True)
    TD_model.save_weights("./tmp/final_weights_SNP_7_1.hdf5",overwrite=True)
...
#    print(newYval)
    ...
    f_matrix1 = confusion_matrix(ny, classes)
    print(np.where(classes != ny)[0])
    print((np.where(classes != ny)[0])%15)
    print((np.where(classes != ny)[0])/15)
    ind_val_0=np.array(ind_val_0)
    print(ind_val_0[(np.where(classes != ny)[0])%15])
    print(Tray[ind_val_0[(np.where(classes != ny)[0])%15]])
    print(BVZ[ind_val_0[(np.where(classes != ny)[0])%15]])
    print(y1[ind_val_0[(np.where(classes != ny)[0])%15]]-1)
...
#    print(newYtest)
    ...
    f_matrix2 = confusion_matrix(ny, classes)
    print(np.where(classes != ny)[0])
    print((np.where(classes != ny)[0])%20)
    print((np.where(classes != ny)[0])/20)
    ...
    TD_model.load_weights("./tmp/weights_SNP_7_1.hdf5")
    TD_model.compile(loss='categorical_crossentropy', optimizer=sgd, metrics=['accuracy'])
    ...
    print(score)
    result3= score[1]
    f_matrix3 = confusion_matrix(ny, classes)
    print(np.where(classes != ny)[0])
    print((np.where(classes != ny)[0])%20)
    print((np.where(classes != ny)[0])/20)
    ind_test_0=np.array(ind_test_0)
...
#print(classes)
    ...
    f_matrix4 = confusion_matrix(ny, classes)
    print(np.where(classes != ny)[0])
    print((np.where(classes != ny)[0])%15)
    print((np.where(classes != ny)[0])/15)
    ind_val_0=np.array(ind_val_0)
