##### This code is developed to extract deep features using cnn, train and test lstm (label each frame of sequence)  ####
...
#            TD_model.compile(loss='categorical_crossentropy', optimizer=sgd, metrics=['accuracy'])
    ...
    if j!= 4 :
        ...
        ind_test_0 = range(0,20)
        ind_val_0 = range(82,97)
        ...
        for kf in range(22) :
            for df in range(4) :
                if newYval[nf,kf,df]==1:
                    ny[nf,kf]=df
    print(ny)
    checkpointer = ModelCheckpoint(filepath="./tmp/weights_SNP_7_eachframe_print.hdf5", verbose=1, save_best_only=True)
...
#            LSTM_model.save_weights("./tmp/final_weights_SNP1.hdf5",overwrite=True)
    TD_model.save_weights("./tmp/final_weights_SNP_7_eachframe_print.hdf5",overwrite=True)
...
#    ny=np.zeros(newYtest.shape[0])
    ...
    TD_model.load_weights("./tmp/weights_SNP_7_eachframe_print.hdf5")
    TD_model.compile(loss='categorical_crossentropy', optimizer=sgd, metrics=['accuracy'])
...
with open('prob_print.csv','a') as f:
    writer = csv.writer(f)
    writer.writerow(('BVZ','Tray_label_single','Plant_label_single'))
    for nff in range (0,20):
        writer.writerow((BVZ[rand00[nff]],Tray[rand00[nff]],y1[rand00[nff]]))
